...
import { updateMenuItem, getCategories, getMenuItems } from "@/api/owner/restaurant";
...
export default function EditMenuItemScreen() {
  const { restaurantId, id } = useLocalSearchParams<{ restaurantId: string; id: string }>();
  ...
  const loadData = async () => {
    try {
      const [categoriesRes, menuItemsRes] = await Promise.all([
        getCategories(),
        getMenuItems(Number(restaurantId)),
      ]);
      setCategories(categoriesRes.categories || []);

      const menuItem = menuItemsRes.menuItems?.find(
      ...
      if (menuItem) {
        ...
        setImage(menuItem.img_url || null);
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateMenuItem(Number(restaurantId), Number(id), formData);
      router.back();
    } catch (error: any) {
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <View className="bg-white px-4 py-3 flex-row items-center border-b border-gray-100">
        <BackButton />
        <Text className="text-xl font-bold ml-4 text-orange-500">Edit Menu Item</Text>
      </View>
